...
#define MAXCOUNTER 	6
#define BUFFERSIZE	MAXCOUNTER + 1
boolean led_state = LOW;		//state of LED, each time an external interrupt 
								//will change the state of LED
unsigned char counter;
unsigned long temp[BUFFERSIZE];
unsigned long sub;
...
void sum() {
 if(data_effect)     {
      heart_rate= 60 * MAXCOUNTER * 1000 /(temp[MAXCOUNTER]-temp[0]);//60*20*1000/20_total_time 
...
void interrupt()
{
    ...
    switch(counter) {
		case 0:
			sub=temp[counter]-temp[MAXCOUNTER];
			Serial.println(sub);
			break;
		default:
    ...
    if (counter==MAXCOUNTER&&data_effect){
		counter=0;
		sum();
    }

    else if(counter!=MAXCOUNTER&&data_effect){
    	counter++;
    	led_state = !led_state;
    }
    else {
...
void arrayInit() {
	for(unsigned char i=0;i < MAXCOUNTER;i ++) {
		temp[i]=0;
	}
	temp[MAXCOUNTER]=millis();
}
